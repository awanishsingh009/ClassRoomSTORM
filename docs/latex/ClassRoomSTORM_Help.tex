% ======================================================================
%  ClassRoomSTORM Studio V1.1 -- User Help
...
\hypersetup{colorlinks=true, linkcolor=blue!45!black, urlcolor=blue!45!black,
  ...
  pdftitle={ClassRoomSTORM Studio V1.1 -- User Help},
...
\newtcolorbox{cstip}[1][Tip]{breakable, colback=green!6, colframe=green!45!black,
  fonttitle=\bfseries, coltitle=green!45!black, title={#1}, boxrule=0.5pt, arc=2pt}
\newtcolorbox{cswarn}[1][Warning]{breakable, colback=red!5, colframe=red!60!black,
  fonttitle=\bfseries, coltitle=red!60!black, title={#1}, boxrule=0.5pt, arc=2pt}
...
\title{\textbf{\studio{} V1.1}\\[2pt]\large User Help --- Virtual and Experimental
       Super-Resolution Microscopy}
\author{Awanish Pratap Singh}
\date{Software version 1.1}
...
\begin{abstract}
\noindent This Help document explains how to install and use \studio{} V1.1, a
desktop teaching application for localization-based super-resolution microscopy.
It is task-oriented: it tells you what each screen and control does. The physics
...
\studio{} V1.1 is a desktop teaching application for \emph{localization-based
...
single-molecule localization microscopy (SMLM) --- the family of methods that
includes STORM and PALM --- in a form that needs no lasers, no fluorescence
...
research-grade analysis package. Its reconstruction deliberately uses a simple,
fully transparent method --- detect bright spots, take an intensity-weighted
centroid, and accumulate --- so that every step is visible and explainable. It
does not perform Gaussian model fitting, maximum-likelihood estimation,
drift correction, or applied physical calibration; those belong to the theory
background and to possible future extensions (see the \emph{Theory} document).
\end{csnote}
...
\begin{description}[style=nextline, leftmargin=0pt]
  ...
  \item[Accumulate.] The centres found in all frames are collected into a single
        image --- the \emph{reconstructed image}. Because it is built from sharp
        centres rather than blurry spots, it is much crisper than the raw video.
  \item[Compare.] The plain average of the video frames (\ui{Raw Mean}) is
...
\begin{figure}[htbp]
  ...
  \caption{The \app{} loop: blink, localize, accumulate, and compare.}
  \label{fig:loop}
\end{figure}
...
file. You compare your reconstruction against it only \emph{afterwards}, to score
your result --- like not peeking at an answer key. Keep \file{truth.csv} in its
...
\app{} V1.1 is a Python desktop application. The user package includes ready-made
...
\emph{Setup completed}. If it fails partway it removes the incomplete environment,
so the launcher never starts from a broken install.
...
\paragraph{Manual installation (advanced).} Instead of the script you may open a
terminal in the package folder and run \file{python -m venv .venv}, activate it
(Windows PowerShell:
\file{.{\textbackslash}.venv{\textbackslash}Scripts{\textbackslash}Activate.ps1};
macOS or Linux: \file{source .venv/bin/activate}), then run
...
the same diagrams with Matplotlib instead, so it is not required. \textbf{NVIDIA
...
V1.1 --- those systems use the CPU.
\end{csnote}
...
\textbf{Windows} --- double-click \file{launch\_studio\_windows.bat};
\textbf{macOS} --- double-click \file{launch\_studio\_macos.command}. The launch
...
\begin{enumerate}
  ...
  \item On the \ui{2 Blinking} page, set \ui{Frames} to about \texttt{300} for a
        quick first run.
  \item On the \ui{3 Image Formation} page, leave the defaults.
...
The pattern defines \emph{where} the source points sit. Choose one of three
...
\begin{figure}[htbp]
  ...
  \caption{A source pattern (a) becomes a set of source points (b). The Virtual
  Experiment switches random subsets of these points on and off to build the
  blinking video.}
...
\textbf{Control} & \textbf{What it does} & \textbf{Default}\\
\midrule
\ui{Frames} & Number of frames in the generated video (1--10000). More frames
  give a denser reconstruction but take longer. & \texttt{1000}\\
\ui{Max blinkers/frame} & The largest number of source points allowed to be on
  ...
  met. & \texttt{30}\\
\ui{Blinker spacing} & How far apart simultaneously-on blinkers must be:
...
If too many blinkers are on at once and they sit close together, their blurred
spots overlap and cannot be told apart. Sparse, well-separated blinking is what
...
This page sets how each source point is rendered into a camera-like image: how
blurred, how bright, and how noisy. \Cref{fig:ve-frame} shows one resulting
frame.
...
\textbf{Control} & \textbf{What it does} & \textbf{Default}\\
\midrule
\ui{sigma\_px} & The blur width $\sigma$ of each spot, in pixels --- the
  point-spread width (0.5--50). Larger values give wider, blurrier spots.
  & \texttt{10.0}\\
...
number of blinkers per frame, so that busy frames stay visible. The scaled value
is shown in the read-only \ui{Effective brightness} field, and \emph{that} value
is what is used to render the frames. If your frames look brighter or dimmer than
...
\begin{figure}[htbp]
  ...
  \caption{A single simulated blinking frame: only a few emitters are on, each
  appearing as a blurred spot on a noisy background.}
  \label{fig:ve-frame}
\end{figure}
...
Two buttons let you check your settings before generating anything:
...
\midrule
\file{blinking\_video.mp4} & The blinking video --- the input for reconstruction.\\
...
may be one you made with the Virtual Experiment or a real recorded blinking
video. The window has six numbered steps: \ui{1 Load Video}, \ui{2 Calibration},
...
preview, and the slider beside it scrubs through the frames --- a quick way to
see the raw blinking data before reconstructing it.

\subsection{Step 2 --- Calibration}
\label{sec:rc-calib}

\begin{csnote}[Calibration is recorded but not yet applied in V1.1]
The \ui{Calibration} page lets you enter a physical scale (units per pixel).
In V1.1, reconstruction runs entirely in \emph{pixel units}: the values on this
page are recorded but are not applied to the results. You can safely leave this
page on its default \ui{Pixels only} setting. Working in pixels is intentional
--- it keeps the method general and avoids unit-conversion errors.
\end{csnote}
...
\begin{csnote}[Two options on this page are not yet active]
The \ui{Reject bad frames} and \ui{Background correction} checkboxes are shown
on the \ui{3 Preprocess} page but have no effect in V1.1. They are reserved for
a future version.
\end{csnote}
...
\begin{description}[leftmargin=1.6em]
  \item[\texttt{single}] One localization per frame --- the single brightest
        region.
  \item[\texttt{multiple}] Several localizations per frame --- each separated
        bright region is located independently.
...
when several well-separated blinkers can be on together. If you are unsure, leave
it on \texttt{auto}. The mode actually used is recorded in \file{summary.json}.
\end{cstip}
...
\ui{Auto CPU+GPU} & Uses the GPU for single blinkers and parallel CPU for
  multiple blinkers.\\
...
many frames are processed --- useful for a quick test run.
...
installed. If these are missing, \app{} does \emph{not} stop --- it quietly runs
on the CPU and records the fallback in \file{summary.json}. If GPU speed
matters, open \file{summary.json} after the run and confirm the GPU was actually
used.
\end{cswarn}
...
On the \ui{6 Results} page you may enter \ui{Cumulative frames} (frame counts at
which to save intermediate reconstructions --- comma-separated whole numbers) and
tick \ui{Compare with truth after run} if a \file{truth.csv} is available. Click
\ui{Run Reconstruction}. When it finishes, the results appear in the browser,
...
The \ui{Linked Profiles} tab, in the \ui{Main Results} group, is the clearest
way to see what reconstruction achieved. It appears when a reconstruction has
...
\begin{itemize}
  \item The \textcolor{red!70!black}{\textbf{red}} curve is the \ui{Raw mean}
        image --- a wide, blurred bump.
  \item The \textcolor{blue!70!black}{\textbf{blue}} curve is the
        \ui{Reconstruction} --- a much narrower peak at the same place.
\end{itemize}

The horizontal axis is the pixel coordinate; the vertical axis is normalized
intensity (each curve scaled to a 0--1 range so the shapes can be compared
directly). A narrow blue peak beside a wide red bump is the visual signature of
super-resolution.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{H8_linked_profile}
  \caption{The linked profile viewer compares the raw and reconstructed images
  along the same line. Red is the raw (diffraction-limited) image; blue is the
  reconstruction.}
  \label{fig:profiles}
\end{figure}
...
\begin{description}[style=nextline, leftmargin=0pt]
  \item[\ui{Raw Mean}] The plain average of all processed frames --- a
        diffraction-limited, blurry image.
  ...
  \item[\ui{Localization Scatter} / \ui{Density Map} / \ui{Raw Overlay}]
        Diagnostic plots: where localizations landed, how densely, and how they
        sit on top of the raw image.
...
\begin{figure}[htbp]
  ...
  \caption{The files written by each workflow. Reconstruction writes both a
  \file{.npy} numerical array and a \file{.png} image for the raw mean and the
  super-resolution image; the linked profile viewer uses the \file{.npy} files.}
  \label{fig:outputs}
\end{figure}
...
\begin{description}[style=nextline, leftmargin=0pt]
  \item[Setup fails to download the packages.]
        This usually means there is no internet access, or a restricted or proxy
        network where Python cannot verify PyPI security certificates (a
  ...
  \item[No bright spots are detected / the reconstruction is empty.]
        Check the video contrast on the first frame. Very dark or very noisy
...
\begin{description}[style=nextline, leftmargin=0pt, itemsep=2pt]
  \item[Blinker] A light source that switches on and off; it appears as a bright
        spot when on.
  \item[Localization] An estimate of the centre position of one bright spot.
  \item[Centroid] The brightness-weighted average position of a spot's pixels;
        the method \app{} uses to localize.
  ...
  \item[Raw mean] The blurry plain average of the video frames.
  \item[Ground truth] The known, exact source positions, saved in
        \file{truth.csv}, used only to score a reconstruction afterwards.
...
Students generate a dataset, reconstruct it, and explore the linked profiles. A
good guided question: \emph{why} is the blue curve narrower than the red one?

\paragraph{Longer project (1--2 weeks).} Add a real recorded blinking video ---
for example from a classroom optical setup or an LED source --- and have
...
\begin{itemize}
  ...
  \item Explore how blur width, noise, blinker density, and overlap affect the
        result.
  \item Treat physical calibration as optional --- the core workflow is
        pixel-first.
\end{itemize}

\paragraph{Suggested parameter sweeps.} Have students vary one setting at a time
and predict the effect before running: \ui{sigma\_px} (blur), \ui{Read noise},
...
{\footnotesize The science behind \app{} --- diffraction, point-spread
functions, blinking, localization, and precision --- is explained in the
